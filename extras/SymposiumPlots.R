...
ggplot(vizData, aes(x = x, y = ease, ymin = easeCi95Lb, ymax = easeCi95Ub, group = adjustment, color = psModel)) +
  ...
  geom_vline(xintercept = x$x + 0.5, color = gray(0.8)) +
  ...
  theme(legend.position = "top",
        ...
        axis.ticks.y = element_line(color = gray(0.8)),
        strip.background = element_blank())

ggsave(file.path(plotsAndTablesFolder, "EASE_crude_all.png"), width = 10, height = 6, dpi = 300)
...
ggplot(vizData, aes(x = x, y = maxSdmMedian, group = x, color = psMethod, fill = psMethod)) +
  geom_vline(xintercept = x$x + 0.5, color = gray(0.8)) +
  ...
  scale_color_manual(values=c("#FCC90C", "#336B91")) +
  scale_fill_manual(values=c("#FCC90C", "#336B91")) +
  ...
  theme(legend.position = "top",
        ...
        axis.ticks.y = element_line(color = gray(0.8)),
        strip.background = element_blank())
ggsave(file.path(plotsAndTablesFolder, "Balance_all.png"), width = 10, height = 6, dpi = 300)
