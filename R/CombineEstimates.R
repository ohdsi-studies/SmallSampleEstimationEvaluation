...
fitOverallOutcomeModel <- function(rowIdx, omrSubset, cmAnalysis, cmFolders) {
  ...
  if (is.null(profiles)) {
    randomFxEstimate <- dplyr::tibble(analysisId = omrSubset$analysisId[rowIdx] + 100,
                                      targetId = omrSubset$targetId[rowIdx],
                                      comparatorId = omrSubset$comparatorId[rowIdx],
                                      outcomeId = omrSubset$outcomeId[rowIdx],
                                      logRr = NA,
    ...
    randomFxEstimate <- dplyr::tibble(analysisId = omrSubset$analysisId[rowIdx] + 100,
                                      ...
                                      p = EmpiricalCalibration::computeTraditionalP(randomFxEstimate$logRr, 
  ...
  getEstimate <- function(outcomeModel) {
    if (is.null(coef(outcomeModel)) || is.na(outcomeModel$outcomeModelTreatmentEstimate$seLogRr)) {
      ...
      return(dplyr::tibble(logRr = outcomeModel$outcomeModelTreatmentEstimate$logRr,
  ...
  if (nrow(ests) == 0) {
    traditionalEstimate <- dplyr::tibble(analysisId = omrSubset$analysisId[rowIdx] + 200,
                                         targetId = omrSubset$targetId[rowIdx],
                                         comparatorId = omrSubset$comparatorId[rowIdx],
                                         outcomeId = omrSubset$outcomeId[rowIdx],
                                         logRr = NA,
    ...
    meta <- meta::metagen(TE = ests$logRr,
                          seTE = ests$seLogRr,
